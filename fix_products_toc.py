...
def fetch_latest_articles():
    req = urllib.request.Request(f"{BASE_URL}/products?limit=25", headers=HEADERS)
    response = urllib.request.urlopen(req)
    data = json.loads(response.read().decode('utf-8'))
    return data.get('news', [])

def fetch_article(a_id):
    req = urllib.request.Request(f"{BASE_URL}/products/{a_id}", headers=HEADERS)
    response = urllib.request.urlopen(req)
    return json.loads(response.read().decode('utf-8'))

def update_article(a_id, new_content):
    payload = json.dumps({'content': new_content}).encode('utf-8')
    req = urllib.request.Request(f"{BASE_URL}/products/{a_id}", data=payload, headers={
        'X-API-Key': API_KEY, 
