...
async function initDb() {
  ...
  try {
    ...
    const newBodyPrompt = `You are a sales expert who understands the psychology of steel foreign trade customers. Based on the title "{title}" and summary "{summary}", write a professional foreign trade marketing long article in ENGLISH for the product {product}.
...
6. You MUST output raw HTML directly. Do NOT output any conversational text or explanation before or after the HTML. Start directly with the first HTML tag and end with the last HTML tag.`;

    db.prepare(`UPDATE ai_post_prompts SET content = ? WHERE type = 'body'`).run(newBodyPrompt);
    console.log('[db] Force migrated all body AI prompts to B2B layout version.');
  } catch (e) {
  ...
  // ── Ship Watchlist ───────────────────────────────────────────────────────
  // Data source tables for the ship tracker feature (removed). Left out of
  // schema creation; pre-existing tables in old databases are inert.
  ...
  if (seoCount === 0) {
    db.prepare(`INSERT INTO seo_settings (site_title, site_description, site_keywords, llms_txt, llms_full_txt) VALUES (?,?,?,?,?)`)
      .run(
  ...
  `)
  try { db.exec('ALTER TABLE live_chat_messages ADD COLUMN is_read INTEGER DEFAULT 0') } catch(e) {}
  try { db.exec('ALTER TABLE live_chat_messages ADD COLUMN timestamp DATETIME DEFAULT CURRENT_TIMESTAMP') } catch(e) {}
  ...
  try { db.exec('ALTER TABLE live_chat_messages ADD COLUMN country_code TEXT') } catch(e) {}
  try { db.exec('ALTER TABLE live_chat_messages ADD COLUMN geo_source TEXT') } catch(e) {}
  try { db.exec('ALTER TABLE live_chat_messages ADD COLUMN geo_resolved_at DATETIME') } catch(e) {}
  ...
  db.exec(`CREATE INDEX IF NOT EXISTS idx_live_chat_ip ON live_chat_messages(ip)`)
