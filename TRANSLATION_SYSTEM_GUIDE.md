...
后台“AI 全站翻译”中的 `reviews` / “产品评价”是独立翻译分组：

- 仅收集状态为 `published` 的真实英文产品评价，待审核和隐藏评价不会进入翻译任务。
- 可翻译字段为评价标题 `review_title`、评价正文 `review_text`，以及激励评价披露 `incentive_disclosure`；姓名、日期、评分和审核状态不翻译。
- 每种目标语言单独保存在 `product_review_translations`。同一评价共享姓名、日期、评分和发布状态，但不同语言正文互相独立。
- 非英语产品页只显示当前语言的完整且有效翻译；缺少翻译时不会回退到英文、中文或其他语言，避免页面混用语言。
- 英文标题、正文或激励披露变化后，原翻译的 `source_hash` 会失效；重新完成该语言翻译前，旧译文不会公开显示。
- 翻译成功写入或移除过期公开译文后，只清理该产品目标语言的 SEO 渲染缓存。缓存清理失败会记录警告，但不会回滚已经保存的译文。

推荐流程：先由管理员审核并发布评价，再选择目标语言和“产品评价”分组执行 AI 翻译，最后切换到对应语言产品详情确认正文与评价 JSON-LD 一致。
